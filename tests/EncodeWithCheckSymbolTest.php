...
class EncodeWithCheckSymbolTest extends TestCase
{
    public static function provideEncodedLengthExpectations(): array
    {
        return [
            '1-bit has length 2' => [2, 0b1],
            '5-bit has length 2' => [2, 0b10000],
            '6-bit has length 3' => [3, 0b1_00000],
            '10-bit has length 3' => [3, 0b10000_00000],
            '11-bit has length 4' => [4, 0b1_00000_00000],
            '15-bit has length 4' => [4, 0b10000_0000_00000],
        ];
    }
    ...
    #[DataProvider('provideEncodedLengthExpectations')]
    public function testAppendsCheckSymbol(int $expectedLength, int $number): void
    {
        $crockfordBase32 = new CrockfordBase32();
        self::assertEquals($expectedLength, strlen($crockfordBase32->encodeWithCheckSymbol($number)));
    }

    #[DataProvider('provideCheckSymbolExpectations')]
    public function testCheckSymbolIsModulo37(string $expectedCheckSymbol, int $number): void
    {
        $crockfordBase32 = new CrockfordBase32();
        $encoded = $crockfordBase32->encodeWithCheckSymbol($number);
        $checkSymbol = substr($encoded, -1);
        self::assertSame($expectedCheckSymbol, $checkSymbol);
    }
}
